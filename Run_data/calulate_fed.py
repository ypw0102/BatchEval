...
def cal_cost(cur,model):
    if '3.5' in model:price = [0.001,0.002]
    elif '4' in model and 'preview' in model:price = [0.01,0.03]
    else: price = [0.03,0.06]
    cost1,cost2 = 0,0
    try:
        tem =cur['raw_data'][0]
        ...
        print(1)
    return cost1/1000*price[0]+cost2/1000*price[1]
...
for metrics in ['Coherent']:
    data_name = "generate_fed_dialog_heterogeneous_{}/".format(metrics[:3].lower())
    data_name_=data_name
    dir_name = data_name
    model = "gpt-4-1106-preview"
    nums_ = list(range(itration_num))
    further_dirs = ["{}_{}_{}_{}_{}_{}".format(metrics, batchsize, temp, n, sub_name, model)]
    ...
    for further_dir in further_dirs:
        ...
        for i in nums:
            ...
            for idx_ in range(len(all_data_dirs)):
                ...
                human_score_list+=[tem['annotations'][metrics] for tem in cur['raw_data']]
